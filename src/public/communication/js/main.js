...
async function socket_function(id) {
    ...
    await startAction();
    let offer = await callAction();
    sendSDP(socket, offer, id, friend_name);
    // await socket.emit('setting_sending_socketID', id);
    // await socket.emit('call_user', friend_username);
    // socket.emit('call_to', {id, friend_name});
};

function sendSDP(socket, offer, id, friend_name) {
    console.log(offer,socket,id,friend_name);
    socket.emit('call_to', {id, friend_name, offer})
}
...
function callAction() {
    ...
    let offer = localPeerConnection.createOffer(offerOptions)
        .then((description) => {
            return createdOffer(description);
            // return offer;
        }).catch(setSessionDescriptionError);
    // console.log('offer',offer);
    // offer.then(result=>{console.log('from offer--->',result)});
    return offer
}
